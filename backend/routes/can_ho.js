...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      res.status(200).json({
        response: "Thêm ảnh thành công", type: true, data: resultHinhAnh
      });
    } catch (error) {
      console.error(error);
...
router.post("/xoa-anh-can-ho", async function (req, res) {
  try {
    ...
    if (resultGetHinhAnh.hinh_anh) {
      ...
      return res.status(200).json({ response: "Xóa ảnh thành công", type: true, data: updatedHinhAnh });
    }

    res.status(400).json({ response: "Lỗi xóa ảnh, không tìm thấy dữ liệu", type: false });
  } catch (err) {
    console.error(err);
...
router.post("/them-can-ho", async (req, res) => {
  try {
    ...
    if (checkMaCanHoExist.length > 0) {
      return res.status(200).json({ response: "Căn hộ đã tồn tại", type: false });
    }
    ...
    await executeQuery(
      sqlCanHo,
      [
        ma_can_ho, chu_can_ho, so_dien_thoai, gia_ban, gia_thue, ten_du_an, dien_tich, so_phong_ngu, so_phong_tam, huong_can_ho, loai_can_ho, noi_that, mo_ta, `${data.ho_ten} đã cập nhật ngày ${now.format("DD/MM/YYYY")}`, trang_thai, ten_toa_nha, truc_can_ho, danh_dau
      ]);

    res.status(200).json({ response: "Thêm căn hộ thành công", type: true });
  } catch (error) {
    console.error(error);
...
router.post("/cap-nhat-can-ho", async (req, res) => {
  try {
    ...
    const sqlCanHo = `
        ...
        loai_can_ho = ?, noi_that = ?,
        ghi_chu = ?, nguoi_cap_nhat = ?,
        trang_thai = ?, danh_dau = ? WHERE id = ?`;

    await executeQuery(
      sqlCanHo, [
      chu_can_ho, so_dien_thoai, gia_ban, gia_thue, ten_du_an, dien_tich, so_phong_ngu, so_phong_tam, huong_can_ho, loai_can_ho, noi_that, mo_ta, `${data.ho_ten} đã cập nhật ngày ${now.format("DD/MM/YYYY")}`, trang_thai, danh_dau, id
    ]);

    res.status(200).json({ response: "Cập nhật căn hộ thành công", type: true });
  } catch (error) {
    console.error(error);
